...
import (
	...
	cometbft "cosmossdk.io/server/v2/cometbft"
...
type (
	AppConstructor     = func(val ValidatorI) serverv2.AppI[transaction.Tx]
...
func New[AppT serverv2.AppI[T], T transaction.Tx](l Logger, baseDir string, cfg Config) (NetworkI, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		cometServer := cometbft.New[serverv2.AppI[transaction.Tx], transaction.Tx](cfg.TxCodec, cometbft.ServerOptions[transaction.Tx]{})
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "simdv2")
		...
		server := serverv2.NewServer(logger, cometServer, grpcServer)
